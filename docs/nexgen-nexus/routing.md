...
NexGen-Nexus uses file-based routing, which means the generated HTML pages are mapped from the directory structure of the source Markdown files. For example, given the following directory structure:
...
There are two important concepts in the file structure of a NexGen-Nexus project: the **project root** and the **source directory**.

### Project Root

Project root is where NexGen-Nexus will try to look for the `.vitepress` special directory. The `.vitepress` directory is a reserved location for NexGen-Nexus' config file, dev server cache, build output, and optional theme customization code.

When you run `vitepress dev` or `vitepress build` from the command line, NexGen-Nexus will use the current working directory as project root. To specify a sub-directory as root, you will need to pass the relative path to the command. For example, if your NexGen-Nexus project is located in `./docs`, you should run `vitepress dev docs`:
...
You can use both absolute and relative paths when linking between pages. Note that although both `.md` and `.html` extensions will work, the best practice is to omit file extensions so that NexGen-Nexus can generate the final URLs based on your config.
...
### Linking to Non-NexGen-Nexus Pages

If you want to link to a page in your site that is not generated by NexGen-Nexus, you'll either need to use the full URL (opens in a new tab) or explicitly specify the target:
...
To serve clean URLs with NexGen-Nexus, server-side support is required.
:::

By default, NexGen-Nexus resolves inbound links to URLs ending with `.html`. However, some users may prefer "Clean URLs" without the `.html` extension - for example, `example.com/path` instead of `example.com/path.html`.
...
If this feature is available to you, you can then also enable NexGen-Nexus' own [`cleanUrls`](../reference/site-config#cleanurls) config option so that:
...
And you want the NexGen-Nexus pages to be generated like this:
...
Since NexGen-Nexus is a static site generator, the possible page paths must be determined at build time. Therefore, a dynamic route page **must** be accompanied by a **paths loader file**. For `packages/[pkg].md`, we will need `packages/[pkg].paths.js` (`.ts` is also supported):
