...
NexGen-Nexus will always use the custom theme instead of the default theme when it detects presence of a theme entry file. You can, however, [extend the default theme](./extending-default-theme) to perform advanced customizations on top of it.

## Theme Interface

A NexGen-Nexus custom theme is defined as an object with the following interface:
...
interface EnhanceAppContext {
  ...
  router: Router // NexGen-Nexus router instance
...
// NexGen-Nexus is pre-configured with @vitejs/plugin-vue.
...
The default export is the only contract for a custom theme, and only the `Layout` property is required. So technically, a NexGen-Nexus theme can be as simple as a single Vue component.
...
3. If your theme requires adjusting the NexGen-Nexus config, export that config under a package sub-path (e.g. `my-theme/config`) so the user can extend it.
...
If the theme requires special NexGen-Nexus config, you will need to also extend it in your own config:
